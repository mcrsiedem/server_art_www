...
const zamowienieUpdatePool = async (req, res) => {
    let connection; // Zmienna dla połączenia z transakcją
    ...
    try {
        ...
        // --- 2. Pobranie połączenia z puli i rozpoczęcie transakcji ---
        connection = await pool.getConnection(); // Pobranie połączenia
        await connection.beginTransaction(); // Rozpoczęcie transakcji
        // 

        // --- 3. Wykonanie wszystkich zapytań (zamieniamy connection.execute na await connection.execute) ---
        ...
        if (daneZamowienia.update == true) {
            ...
            await connection.execute(sql, dane);
        ...
        // ... (Usuwanie produktów, jeśli jest potrzebne, ale brak go w oryginalnym kodzie) ...
        ...
        for (let row of elementy.filter(x => x.insert == true && x.delete != true)) {
            ...
            await connection.execute(sql2, dane2);

            // -- Wstawianie do technologie_elementy --
            if (technologieID != null && technologieID.length > 0) {
                for (let tech_id of technologieID) {
                    let dane_tech = [row.id, row.zamowienie_id, tech_id.technologia_id, row.produkt_id, row.nazwa, row.typ, row.ilosc_stron, row.format_x, row.format_y, row.papier_id, row.papier_postac_id, row.naklad, row.uwagi, row.stan, row.status, row.etap, row.indeks];
                    var sql_tech = "INSERT INTO artdruk.technologie_elementy (id,zamowienie_id,technologia_id,produkt_id,nazwa,typ,ilosc_stron,format_x,format_y,papier_id,papier_postac_id,naklad,uwagi,stan,status,etap,indeks) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?); ";
                    await connection.execute(sql_tech, dane_tech);
                }
            }
        }
        ...
        for (let row of faktury.filter(x => x.insert == true && x.delete != true)) {
            ...
            var sql = "INSERT INTO artdruk.zamowienia_faktury (id,zamowienie_id,indeks,nazwa,wz,ilosc,cena,suma,info,status,stan,dodal) values (?,?,?,?,?,?,?,?,?,?,?,?); ";
            await connection.execute(sql, data);
        }
        ...
        // --- 5. WYCOFANIE transakcji w przypadku błędu ---
        console.error("Błąd podczas aktualizacji zamówienia:", err);
        if (connection) {
            try {
                await connection.rollback();
                console.log("Transakcja wycofana (rollback).");
            } catch (rollbackErr) {
                console.error("Błąd podczas wycofywania transakcji:", rollbackErr);
            }
        }
        res.status(500).json({
            message: "Wystąpił błąd podczas zapisywania zamówienia. Transakcja została anulowana.",
            error: err.message
        });

    } finally {
        // --- 6. ZWOLNIENIE połączenia z powrotem do puli ---
        if (connection) {
            connection.release(); // Zwraca połączenie do puli
            // console.log("Połączenie zwrócone do puli.");
        }
    }
}

module.exports = {
    zamowienieUpdatePool
}