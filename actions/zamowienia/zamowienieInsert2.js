...
const zamowienieInsert = async (req, res) => {
  ...
  try {
    ...
    for (let p of produkty) {
      await conn.execute("INSERT INTO artdruk.zamowienia_produkty (id,zamowienie_id,nazwa,wersja,opiekun_zamowienia_id,uwagi,stan,status,etap,typ,ilosc_stron,format_x,format_y,oprawa,naklad,indeks) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", 
    ...
    for (let e of elementy.filter(x => x.delete != true)) {
      await conn.execute("INSERT INTO artdruk.zamowienia_elementy (id,zamowienie_id,produkt_id,nazwa,typ,ilosc_stron,kolory,format_x,format_y,papier_id,papier_postac_id,naklad,info,uwagi,stan,status,etap,tytul,papier_info,indeks) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
      ...
      await conn.execute("INSERT INTO artdruk.zamowienia_pliki (id,zamowienie_id,produkt_id,element_id,uwagi,stan,status,etap,indeks) values (?,?,?,?,?,?,?,?,?)",
      [e.id, zamowienie_id, e.produkt_id, e.id, e.uwagi, e.stan, e.status, 2, e.indeks]);
      resultsForFront.push([{ zapis: true }]);
    }

    for (let f of fragmenty.filter(x => x.delete != true)) {
      await conn.execute("INSERT INTO artdruk.zamowienia_fragmenty (id,zamowienie_id,produkt_id,element_id,oprawa_id,naklad,ilosc_stron,wersja,info,typ,indeks) values (?,?,?,?,?,?,?,?,?,?,?)",
    ...
    for (let o of oprawa.filter(x => x.delete != true)) {
      await conn.execute("INSERT INTO artdruk.zamowienia_oprawa (id,zamowienie_id,produkt_id,oprawa,naklad,bok_oprawy,data_spedycji,uwagi,wersja,data_czystodrukow,indeks) values (?,?,?,?,?,?,?,?,?,?,?)",
    ...
    for (let pe of procesyElementow.filter(x => x.delete != true)) {
      await conn.execute("INSERT INTO artdruk.zamowienia_procesy_elementow (id,zamowienie_id,ilosc_uzytkow,produkt_id,element_id,proces_id,front_ilosc,back_ilosc,front_kolor,back_kolor,info,nazwa_id,indeks) values (?,?,?,?,?,?,?,?,?,?,?,?,?)",
    ...
    for (let pp of procesyProduktow.filter(x => x.delete != true)) {
      await conn.execute("INSERT INTO artdruk.zamowienia_procesy_produktow (id,indeks,utworzyl,zamowienie_id,oprawa_id,proces_id,nazwa_id,naklad,ilosc_uzytkow,info) values (?,?,?,?,?,?,?,?,?,?)",
    ...
    for (let pak of pakowanie) {
      await conn.execute("INSERT INTO artdruk.zamowienia_pakowanie(id,zamowienie_id,produkt_id,nazwa,naklad,uwagi,sztuki_w_paczce,rodzaj_pakowania,indeks) values (?,?,?,?,?,?,?,?,?)",
      [pak.id, zamowienie_id, pak.produkt_id, pak.nazwa, pak.naklad, pak.uwagi, pak.sztuki_w_paczce, pak.rodzaj_pakowania, pak.indeks]);
      resultsForFront.push([{ zapis: true }]);
    }

    await conn.execute("INSERT INTO artdruk.zamowienia_ksiegowosc (zamowienie_id,koszty_status,koszty_wartosc,faktury_status,faktury_wartosc,faktury_naklad,info) values (?,?,?,?,?,?,?)",
    [zamowienie_id, 1, "", 1, "", ksiegowosc.faktury_naklad, ksiegowosc.info]);
    resultsForFront.push([{ zapis: true }, { zamowienie_id: zamowienie_id }]);

    await conn.execute("INSERT INTO artdruk.oddania_grupy (zamowienie_id,id,status,typ) values (?,?,?,?)", [zamowienie_id, 1, 1, 1]);
    resultsForFront.push([{ zapis: true }]);
