...
let id =  DecodeToken(token).id;
  ...
  try {
    const [rows] = await pool.execute("SELECT count(*) as ile FROM artdruk.view_zamowienia_2 where etap = 16 and koszty_status =1 and faktury_status =1 and opiekun_id = ?", [id]) 
