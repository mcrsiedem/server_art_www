...
const zamowienieUpdatePool = async (req, res) => {
    let conn; // Zmienna dla połączenia z transakcją
    ...
    try {
        ...
        conn = await pool.getConnection(); // Pobranie połączenia
        await conn.beginTransaction(); // Rozpoczęcie transakcji
        // 

        // --- 3. Wykonanie wszystkich zapytań (zamieniamy conn.execute na await conn.execute) ---
        ...
        for (let row of elementy.filter(x => x.insert == true && x.delete != true)) {
            ...
            if (technologieID != null && technologieID.length > 0) {
                for (let tech_id of technologieID) {
                    ...
                    await conn.execute(sql_tech, dane_tech);
                }
            }
        }
        ...
        if (conn) {
            conn.release(); // Zwraca połączenie do puli
