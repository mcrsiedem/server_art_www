...
const { zamowienieInsert } = require('./zamowienia/zapis/zamowienieInsert');
...
const { aktualizujGrupeOprawaUwagi } = require('./oprawa/aktualizujGrupeOprawaUwagi');
...
const { zamowienieUpdatePool } = require('./zamowienia/zapis/zamowienieUpdatePool');
...
router.get('/zestawienie_klienci_wartosc/:od/:do/:token',verifyTokenParams('zestawienia'),getZestawienieKlienciWartosc);     
...
router.get('/skasujTechnologie/:id_delete/:zamowienie_id/:user_id/:token',verifyTokenParams('technologia_zapis'),usunTechnologie); //stare
