...
const usunTechnologie = async (req, res) => {
    ...
    const { technologia_id, zamowienie_id, user_id } = req.params;
    ...
    try {
        ...
        console.log(`Wywołuję procedurę kasowania technologii. ID: ${technologia_id}, Zamówienie: ${zamowienie_id}, User: ${user_id}`);

        // W przypadku CALL, execute zwraca tablicę, gdzie pierwszy element to wyniki procedury
        const [result] = await conn.execute(sql, [technologia_id, zamowienie_id, user_id]);
