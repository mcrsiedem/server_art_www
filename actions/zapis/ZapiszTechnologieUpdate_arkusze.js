...
const zapiszTechnologieUpdate_arkusze=(arkusze) =>{
...
for(let row of arkusze.filter(x => x.update == true && x.insert != true) ){
  ...
  for(let row of arkusze.filter(x => x.insert == true && x.delete != true) ){
    var sql =
    "INSERT INTO artdruk.technologie_arkusze (id,indeks,technologia_id,typ_elementu,rodzaj_arkusza,nr_arkusza,element_id,ilosc_stron,ilosc_leg,naklad,uwagi) " +
    "values ('" +
    row.id +  "','" +
    row.indeks +        "','" +
    row.technologia_id +        "','" +
